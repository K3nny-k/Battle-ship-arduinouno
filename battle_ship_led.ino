...
#define JOYSTICK_BUTTON_PIN 2  // Joystick button pin
...
// Joystick button state variables
...
unsigned long joystickDebounceDelay = 50; // 50 ms debounce

// Joystick double-click variables
...
void setup() {
  ...
  lcd.setCursor(0, 1);
  lcd.print(F("Press JS Btn"));

  Serial.println(F("Battleship Game"));
  Serial.println(F("Press the joystick button to start."));
}
...
void loop() {
  ...
  // Check for joystick button state change
  if (joystickButtonState != lastJoystickButtonState) {
    if (joystickButtonState == LOW) {
      ...
      if (pressDuration >= 2000) {
        ...
        joystickButtonShortPress = true;
        // Handle double-click
        unsigned long timeSinceLastRelease =
          millis() - lastJoystickButtonReleaseTime;
        if (timeSinceLastRelease < joystickDoubleClickThreshold) {
          joystickButtonClickCount++;
        } else {
          joystickButtonClickCount = 1;
        }
        lastJoystickButtonReleaseTime = millis();
      }
  ...
  // Handle long press
  if (joystickButtonState == LOW && !joystickButtonLongPressHandled) {
    unsigned long pressDuration = millis() - joystickButtonPressTime;
    if (pressDuration >= 2000) { // Long press threshold
      // Long press detected
      resetGame();
      playBuzzerTone(1000, 200);
      joystickButtonLongPressHandled = true;
      joystickButtonShortPress = false; // Prevent short press
    }
  }

  // Handle short press actions
  if (joystickButtonShortPress) {
    joystickButtonShortPress = false; // Reset flag
...
void playerPlaceShips() {
  ...
  // Handle joystick button for placing ships and toggling orientation
  if (joystickButtonShortPress) {
    joystickButtonShortPress = false; // Reset flag
    ...
    if (currentTime - lastPressTime < joystickDoubleClickThreshold) {
      pressCount++;
    } else {
      pressCount = 1;
    }
    ...
    if (pressCount == 2) {
      ...
      // Single click, attempt to place ship
      if (canPlaceShip(playerGrid, cursorX, cursorY,
          currentShip.size, playerShipHorizontal)) {
        placeShip(playerGrid, cursorX, cursorY, currentShip.size,
          playerShipHorizontal);
        shipId++;
...
void playerAttack() {
  ...
  // Handle joystick button for confirming attacks
  if (joystickButtonShortPress) {
    joystickButtonShortPress = false; // Reset flag
